...
export (bool) var preventRespawnOnMaxProjectilesLive=false #true means if you exceed preojctile instance max, trying to create another does nothgin. gotta wait for one of the live projectiles to end
...
#override parent	
func init(_playerController,_spriteAnimation):
...
func signalProjectileCreation():
	...
	#don't create projectiles when max on battlefields and flag enabled to prevent
	if preventRespawnOnMaxProjectilesLive:
		var numActiveProjectiles=0
		#count number of active proejectiles
		for li in liveInstances:
			if li !=null:
				numActiveProjectiles = numActiveProjectiles+1
			
		if numActiveProjectiles >= maxNumProjectiles:
			return 
...
func attemptSignalProjectileCreation():
